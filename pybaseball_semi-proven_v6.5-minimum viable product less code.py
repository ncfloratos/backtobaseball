...
def extract_player_name(batter_id, id_to_name):
...
def parse_play_description(events, description, batter):
    ...
    cleaned_fielders=extract_fielders(description)
    ...
    # **Explicitly Recognize Common Plays**
   ...
   # **Flyouts & Groundouts**
    ...
    if 'grounded_into_double_play' in events:
        return parse_grounded_into_double_play(cleaned_fielders)
    
    #Double-Plays
    if 'double_play' in events:
        return parse_double_play(cleaned_fielders)
    
    #Force-Outs
    if 'force_out' in events:
        return parse_force_out(cleaned_fielders)
    
    #Fielders Choices
    if 'fielders_choice' in events:
        return parse_fielders_choice(cleaned_fielders)
    
    #Reached-On Errors
    if 'field_error' in events:
        return parse_reached_on_error(cleaned_fielders)

    # **Groundouts**
    if ('grounds out' in description or 'groundout' in description):
        return parse_groundouts(cleaned_fielders)
...
def extract_fielders(description):
    ...
    # If description is missing or not a string, bail out
    ...
    involved_fielders = [fielders[f] for f in fielders_found if f in fielders]
      ...
      # If no fielders were found, just return empty list
...
def parse_grounded_into_double_play(cleaned_fielders):
...
def parse_double_play(cleaned_fielders):
...
def parse_force_out(cleaned_fielders):
...
def parse_fielders_choice(cleaned_fielders):
...
def parse_groundouts(cleaned_fielders):

    # ✅ Ensure groundout abbreviation follows correct order
    if len(cleaned_fielders) >= 2:
        return f"GO{'-'.join(cleaned_fielders)}"
    elif len(cleaned_fielders) == 1:
        return f"GO{cleaned_fielders[0]}U"  # Unassisted Groundout
    
    return "GO"

def parse_reached_on_error(description, cleaned_fielders):
    ...
    if len(cleaned_fielders) >= 2:
        return f"E{error_type}{'-'.join(cleaned_fielders)}"  # Multiple fielders involved
    elif len(cleaned_fielders) == 1:
        return f"E{error_type}{cleaned_fielders[0]}"  # Single fielder error
...
# Generate the PDF
def save_combined_scorecard(df, output_pdf,title=None, play_by_play_data=None, id_to_name=None):
    pdfmetrics.registerFont(TTFont('Unifont', "C:/Users/ncflo/.matplotlib/Unifont.ttf"))
...
def process_play_by_play(df):
    ...
    all_player_ids = pd.concat([
        df['batter'].dropna().astype(int),
        df['pitcher'].dropna().astype(int)
    ]).unique()

# Lookup all players in one call
    player_info = playerid_reverse_lookup(all_player_ids, key_type='mlbam')

# Build the dictionary ID → "First Last"
    id_to_name = {
        row['key_mlbam']: f"{row['name_first']} {row['name_last']}"
        for _, row in player_info.iterrows()
    }

    # ✅ Add batter_name to df_filtered so everyone can use it
    df_filtered['batter_name'] = df_filtered['batter'].map(id_to_name)
    ...
    # Initialize batter stats
        ...
        team_data['batter_name'] = team_data.apply(lambda row: extract_player_name(row['batter'], id_to_name), axis=1)
...
if __name__ == "__main__":
    ...
    if df is not None:
        ...
        save_combined_scorecard(team_scorecards, output_pdf, title=title_text, play_by_play_data=play_by_play_data, id_to_name=id_to_name)  # ✅ Pass play_by_play_data
